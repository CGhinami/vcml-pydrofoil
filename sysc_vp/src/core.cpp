 ...
 ******************************************************************************/
 ...
 {
     switch(irq) {
     case MEIP: bit = MEIP_BIT; return true;
     case SEIP: bit = SEIP_BIT; return true;
     case MSIP: bit = MSIP_BIT; return true;
     case MTIP: bit = MTIP_BIT; return true;
 ...
 {
     size_t bit = 0;
     if(!mip_bit_for_irq(irq, bit)) {
         mwr::log_warn("Hart %lu: ignoring unknown irq %zu", m_hart_id, irq);
         return;
     }

     size_t encoded = (static_cast<uint32_t>(bit) & IRQ_MIP_BIT_MASK) | (set ? IRQ_MIP_SET_FLAG : 0u);
 ...
 void PydrofoilCore::simulate(size_t cycles)
 {
     ...
     {
         std::lock_guard<std::mutex> lock(irq_mutex);
         pending_irqs.swap(irq_events);
     }

     while(!pending_irqs.empty()) {
         notify_pending_irq(pending_irqs.front().first, pending_irqs.front().second);
         pending_irqs.pop();
     }

     {
         std::lock_guard<std::mutex> lock(memtask_mutex);
         sim_done_flag = false;
     }

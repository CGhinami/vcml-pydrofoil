...
#if NRCPU > 4
    // CORE 4
    tlm_bind(m_bus, m_core4, "insn");
    tlm_bind(m_bus, m_core4, "data");
    clk_bind(m_clock_cpu, "clk", m_core4, "clk");
    gpio_bind(m_reset, "rst", m_core4, "rst");
    m_plic.irqt[4].bind(m_core4.irq[0]);
    m_clint.irq_sw[4].bind(m_core4.irq[core::MSIP]);
    m_clint.irq_timer[4].bind(m_core4.irq[core::MTIP]);

    // CORE 5
    tlm_bind(m_bus, m_core5, "insn");
    tlm_bind(m_bus, m_core5, "data");
    clk_bind(m_clock_cpu, "clk", m_core5, "clk");
    gpio_bind(m_reset, "rst", m_core5, "rst");
    m_plic.irqt[5].bind(m_core5.irq[0]);
    m_clint.irq_sw[5].bind(m_core5.irq[core::MSIP]);
    m_clint.irq_timer[5].bind(m_core5.irq[core::MTIP]);

    // CORE 6
    tlm_bind(m_bus, m_core6, "insn");
    tlm_bind(m_bus, m_core6, "data");
    clk_bind(m_clock_cpu, "clk", m_core6, "clk");
    gpio_bind(m_reset, "rst", m_core6, "rst");
    m_plic.irqt[6].bind(m_core6.irq[0]);
    m_clint.irq_sw[6].bind(m_core6.irq[core::MSIP]);
    m_clint.irq_timer[6].bind(m_core6.irq[core::MTIP]);

    // CORE 7
    tlm_bind(m_bus, m_core7, "insn");
    tlm_bind(m_bus, m_core7, "data");
    clk_bind(m_clock_cpu, "clk", m_core7, "clk");
    gpio_bind(m_reset, "rst", m_core7, "rst");
    m_plic.irqt[7].bind(m_core7.irq[0]);
    m_clint.irq_sw[7].bind(m_core7.irq[core::MSIP]);
    m_clint.irq_timer[7].bind(m_core7.irq[core::MTIP]);
#endif
}
...
int system::run()
{
    ...
    vcml::log_info("total");
    vcml::log_info("  number of cores  : %d", NRCPU);
    vcml::log_info("  duration         : %.9fs", duration);
    vcml::log_info("  runtime          : %.4fs", realtime);
    vcml::log_info("  cycles per core  : %llu", m_core0.cycle_count());
    vcml::log_info("  nins             : %llu", ninsn); // Fixed typo and colon placement

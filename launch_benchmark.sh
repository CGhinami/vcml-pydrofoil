...
# without gdb: ./launch.sh <path to myconfig.cfg> [extra args...]
# with gdb: ./launch.sh -d <path to myconfig.cfg> [extra args...]
...
if [[ -z "$1" ]]; then
    echo "Usage: $0 [-d|--debug] <config-file> [extra args...]"
    exit 1
fi

VP_CFG=$1
shift # <--- ADDED: Shift again so "$@" now contains all the extra arguments (like -c n_cores=8)
